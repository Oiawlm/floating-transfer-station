...
复盘标签每天对应一个本地 Markdown 文件，保存在数据目录的 `reviews/yyyy-MM-dd.md`。打开标签即可编辑今天的内容，输入停止后自动保存；历史日期可从日期列表切换。文件由 Obsidian 或其他编辑器修改时，应用会刷新；同时编辑会提示保留本地、载入文件或合并。复盘标签不会接收剪贴板或拖放内容。
