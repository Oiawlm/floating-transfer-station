...
Mac 本地开发使用 `FloatingTransferStation.Mac.slnx`，单独打包使用 `scripts/build-macos.ps1`（仅 `osx-arm64`）；Mac 原生执行 `-RuntimeIdentifier osx-arm64 -SmokeTest` 可验证包内启动并生成展开/收起截图。本机构建不能代替 Mac 实机验证，包旁 JSON 明确记录此状态。Windows 端取证可用 `FTS_PREVIEW_DATA_DIR`（隔离数据目录 + 独立单实例锁）与可选 `FTS_PREVIEW_THEME=dark|light` 启动真实应用截图，不影响已安装应用的数据。
